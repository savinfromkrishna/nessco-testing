...
export async function generateMetadata({
  ...
  if (!post || !countryName) {
    return {
      ...
      openGraph: {
        title: "Blog Post Not Found",
        description: "The requested blog post could not be found.",
        url: `${baseUrl}/${locale}/know-your-machine/${id}`,
        images: [],
      },
      twitter: {
  ...
  return {
    ...
    openGraph: {
      type: "article",
      siteName: "Nessco",
      url: `${baseUrl}/${locale}/know-your-machine/${post.slug}`,
      title: `${post.title} - ${countryName}`,
      description: post.excerpt,
      images: [
        {
    ...
    alternates: {
      canonical: `${baseUrl}/${locale}/know-your-machine/${post.slug}`,
    },
    twitter: {
      card: "summary_large_image",
      site: "@NesscoIndia",
...
export default async function KnowYourMachinePage({
  params: { locale, id },
}: Props) {
  if (!locales.includes(locale as any)) {
    locale = "en";
